            ...
            if file.endswith('.py'):
                ...
                with open(file_path, encoding='utf-8', errors='ignore') as f:
                    for i, line in enumerate(f):
                        if any(
                            term in line for term in py_debug_terms
                        ) and not line.strip().startswith('#'):
                            found_issues.append(
                                f"   - Python Debug: Found in '{file_path}' on line {i + 1}"
                            )
                ...
                with open(file_path, encoding='utf-8', errors='ignore') as f:
                    for i, line in enumerate(f):
                        if any(
                            term in line for term in js_debug_terms
                        ) and not line.strip().startswith('//'):
                            found_issues.append(
                                f"   - JS Debug: Found in '{file_path}' on line {i + 1}"
                            )
...
def process_doctype_permissions():
    ...
    docperms = frappe.get_all(
        'DocPerm', filters={'parent': ('in', app_doctypes)}, fields=['parent', 'role'] + PERM_TYPES
    )
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(
            'Ensure you have set the correct site name in the script and are running it via `bench execute`.'
        )
    finally:
        frappe.destroy()
