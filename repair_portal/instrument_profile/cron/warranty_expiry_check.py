...
def load_warranty_check_config() -> Dict[str, Any]:
    ...
    try:
        ...
        config = {
            ...
            "expiry_threshold_days": getattr(
                settings, "warranty_expiry_threshold_days", DEFAULT_EXPIRY_THRESHOLD_DAYS
            ),
            "early_warning_days": getattr(
                settings, "warranty_early_warning_days", DEFAULT_EARLY_WARNING_DAYS
            ),
            "enable_customer_notifications": getattr(
                settings, "enable_warranty_customer_notifications", True
            ),
            ...
            "enable_performance_monitoring": getattr(
                settings, "enable_warranty_performance_monitoring", True
            ),
    ...
    config["early_warning_days"] = max(
        config["expiry_threshold_days"], min(cint(config["early_warning_days"]), 730)
    )
...
def execute_warranty_check_batched(config: Dict[str, Any], job_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        while True:
            ...
            instruments_batch = get_warranty_candidates_batch(today, early_warning_date, batch_size, offset)
...
def get_warranty_candidates_batch(
    today: str, early_warning_date: str, batch_size: int, offset: int
) -> List[Dict[str, Any]]:
...
def process_single_instrument(
    instrument: Dict[str, Any], config: Dict[str, Any], today: str, expiry_threshold_date: str, job_id: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not should_send_notification(instrument["name"], category):
            ...
            return {"category": category, "notification_sent": False, "days_until_expiry": days_until_expiry}
...
def send_admin_notification(
    ...
    frappe.sendmail(
        recipients=allowed_recipients, subject=subject, message=message, delayed=False, retry=MAX_RETRIES
    )

    if throttled:
        frappe.logger("warranty_cron").info(f"Skipped throttled admin recipients for {category}: {throttled}")
...
def send_customer_notification(
    ...
    if not _within_daily_limit(customer_email, f"customer:{category}", limit):
        frappe.logger("warranty_cron").info(f"Customer {customer_email} throttled for category {category}")
        return

    frappe.sendmail(
        recipients=[customer_email], subject=subject, message=message, delayed=True, retry=MAX_RETRIES
    )
    ...
    report = {
        ...
        "performance": {
            ...
            "average_batch_size": round(
                result["total_instruments_checked"] / max(result["batches_processed"], 1), 2
            ),
        },
...
def send_job_summary_email(report: Dict[str, Any]):
    ...
    try:
        ...
        frappe.sendmail(
            recipients=["admin@artisanclarinets.com"], subject=subject, message=message, delayed=True
        )
...
def send_job_failure_notification(job_id: str, error_message: str):
    ...
    try:
        ...
        frappe.sendmail(
            recipients=["admin@artisanclarinets.com"], subject=subject, message=message, delayed=False
        )
