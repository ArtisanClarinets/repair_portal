...
try:
    ...
    def ensure_serial_document(serial_input: str, instrument: str) -> Optional[str]:  # type: ignore
        return None
...
def _get_isn(instrument: frappe._dict) -> Optional[frappe._dict]:
...
def _get_owner_details(customer: Optional[str]) -> Optional[frappe._dict]:
    if not customer:
        return None
    return frappe.db.get_value(
        "Customer",
...
def _headline(brand: Optional[str], model: Optional[str], serial_no: Optional[str]) -> str:
    b = (brand or "").strip()
    m = (model or "").strip()
...
def sync_profile(profile_name: str) -> Dict[str, str]:
    ...
    try:
        ...
        _safe_set_scalar(profile.name, "headline", _headline(instrument.brand, instrument.model, instrument.serial_no))
...
@frappe.whitelist()
def sync_now(profile: Optional[str] = None, instrument: Optional[str] = None) -> Dict[str, str]:
...
def on_linked_doc_change(doc, method=None):
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), f"Instrument Profile sync failed for instrument {instrument}")
...
def _aggregate_snapshot(instrument_name: str, profile_name: str) -> Dict[str, object]:
    ...
    accessories = _collection_by_instrument(
        ...
        field_candidates=[
            "accessory_type", "type", "description", "acquired_on", "removed_on", "paired_with"
        ],
    ...
    conditions = _collection_by_instrument(
        ...
        instrument_name=instrument_name,
        field_candidates=["recorded_on", "condition_score", "notes", "technician", "workflow_state"],
...
def get_snapshot(instrument: Optional[str] = None, profile: Optional[str] = None) -> Dict[str, object]:
    """
