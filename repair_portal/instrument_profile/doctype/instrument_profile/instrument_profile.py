...
Changes (2025-07-17):
- Improved exception handling: granular excepts, logs all errors, never swallows exceptions silently.
- All child table refreshes are atomic—partial failures are logged for audit.
- Warranty expiration field now synced from Serial No on update/validate.
- Updated file header and docstring to match project template.
"""
    ...
    def on_update(self) -> None:
        ...
        try:
            ...
            try:
                ...
                frappe.log_error(
                    frappe.get_traceback(), 'InstrumentProfile: Serial No insert failed'
                )
        except Exception:
        ...
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(), 'InstrumentProfile: Warranty Expiration Sync Failed'
            )
