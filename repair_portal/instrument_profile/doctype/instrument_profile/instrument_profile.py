...
class InstrumentProfile(Document):
    ...
    def before_insert(self):
        ...
        try:
            ...
            frappe.logger("instrument_profile").error(
                f"Failed to create Instrument Profile: {str(e)}"
            )
            raise
    ...
    def on_update(self):
        ...
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(), f'InstrumentProfile: sync failed ({self.name})'
            )
        finally:
    ...
    def after_insert(self):
        ...
        try:
            frappe.logger("instrument_profile").info(
                f"Successfully created Instrument Profile {self.name}"
            )
            ...
            frappe.log_error(
                frappe.get_traceback(), f'InstrumentProfile: initial sync failed ({self.name})'
            )
        finally:
            frappe.flags.in_profile_sync = False
    ...
    def _validate_field_permissions(self):
        ...
        for field, required_roles in restricted_fields.items():
            if self.has_value_changed(field) and not any(role in user_roles for role in required_roles):
                frappe.throw(
                    _('Insufficient permissions to modify field: {0}').format(field)
                )
    ...
    def _validate_image_file(self, file_url: str, field_name: str):
        ...
        try:
            ...
            if file_doc.file_size and file_doc.file_size > (MAX_PHOTO_SIZE_MB * 1024 * 1024):
                raise ValidationError(
                    f"File size for {field_name} exceeds {MAX_PHOTO_SIZE_MB}MB limit"
                )
    ...
    def _log_field_changes(self):
        ...
        significant_fields = [
            'customer', 'status', 'workflow_state', 'current_location',
            'warranty_end_date', 'instrument_model'
        ]
        
        significant_changes = {
            field: {
                'old': self.get_db_value(field),
                'new': self.get(field)
            }
...
def get_instrument_profile_summary(name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        frappe.logger("instrument_profile").error(
            f"Failed to get profile summary for {name}: {str(e)}"
        )
...
def update_instrument_location(name: str, new_location: str) -> Dict[str, Any]:
    ...
    try:
        ...
        frappe.logger("instrument_profile").error(
            f"Failed to update location for {name}: {str(e)}"
        )
