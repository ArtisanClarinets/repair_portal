...
class TestInstrumentCategory(FrappeTestCase):
    ...
    def test_instrument_category_creation(self):
        ...
        category = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Test Category 1", "is_active": 1}
        )
        category.insert()
    ...
    def test_required_field_validation(self):
        ...
        with self.assertRaises(frappe.MandatoryError):
            frappe.get_doc({"doctype": "Instrument Category", "is_active": 1}).insert()

    def test_is_active_default_value(self):
        """Test that is_active defaults to 1"""
        category = frappe.get_doc({"doctype": "Instrument Category", "title": "Test Category 2"})
        category.insert()
    ...
    def test_inactive_category_creation(self):
        ...
        category = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Test Category Inactive", "is_active": 0}
        )
        category.insert()
    ...
    def test_category_naming(self):
        ...
        category = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Test Category Unique Name", "is_active": 1}
        )
        category.insert()
    ...
    def test_title_uniqueness(self):
        ...
        category1 = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Duplicate Title Test", "is_active": 1}
        )
        category1.insert()

        # Create second category with same title - should work
        category2 = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Duplicate Title Test", "is_active": 0}
        )
        category2.insert()
    ...
    def test_title_validation(self):
        ...
        with self.assertRaises(frappe.MandatoryError):
            frappe.get_doc({"doctype": "Instrument Category", "title": "", "is_active": 1}).insert()

        # None title should fail
        with self.assertRaises(frappe.MandatoryError):
            frappe.get_doc({"doctype": "Instrument Category", "title": None, "is_active": 1}).insert()

    def test_category_state_changes(self):
        """Test changing active state"""
        category = frappe.get_doc(
            {"doctype": "Instrument Category", "title": "Test Category State Change", "is_active": 1}
        )
        category.insert()
    ...
    def test_various_title_formats(self):
        ...
        for i, title in enumerate(titles):
            category = frappe.get_doc(
                {"doctype": "Instrument Category", "title": f"{title} Test {i}", "is_active": 1}
            )
            category.insert()
