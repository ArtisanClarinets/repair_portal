...
class Instrument(Document):
    ...
    def check_duplicate_serial_no(self):
        ...
        same = frappe.db.exists(
            'Instrument', {'serial_no': current_value, 'name': ['!=', self.name]}
        )
        if same:
            frappe.log_error(
                f'Duplicate Serial Number (exact match): {current_value} in Instrument {same}.'
            )
            frappe.throw(
    ...
    def ensure_valid_instrument_category(self):
        ...
        if self.instrument_category and not frappe.db.exists(
