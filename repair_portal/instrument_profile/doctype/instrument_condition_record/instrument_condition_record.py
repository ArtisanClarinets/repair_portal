...
class InstrumentConditionRecord(Document):
    ...
    def update_instrument_status(self):
        ...
        try:
            ...
            instrument.save()
            frappe.msgprint(f"Status of Instrument <strong>{self.instrument}</strong> updated to 'Available'.")
        except frappe.DoesNotExistError:
            frappe.msgprint(f"Could not find Instrument {self.instrument} to update its status.", indicator="orange")