...
report={'meta':{'root':root,'files_found':len(paths)},'files':{},'doctypes':{},'issues':[]}
...
for dtname,dt in report['doctypes'].items():
    ...
    for key,items in by_fn_case.items():
        if len(items)>1:
            report['issues'].append({'type':'intra_doctype_duplicate_fieldname_case_insensitive','doctype':dtname,'fieldname_variant':key,'occurrences':items})
    # duplicate labels case-insensitive
    for key,items in by_lbl_case.items():
        if len(items)>1:
            report['issues'].append({'type':'intra_doctype_duplicate_label_case_insensitive','doctype':dtname,'label_variant':key,'occurrences':items})
    ...
    for key,items in by_fn_canon.items():
        ...
        if len(names)>1:
            ...
            report['issues'].append({'type':'intra_doctype_similar_fieldnames','doctype':dtname,'canonical_compare':key,'names':list(names),'suggested_fieldname':suggestion,'occurrences':items})
...
for dtname,dt in report['doctypes'].items():
    for f in dt['fields']:
        fn=f.get('fieldname')
        lbl=f.get('label')
        cross[canon_compare_fieldname(fn)].append({'doctype':dtname,'fieldname':fn,'label':lbl,'path':f['path']})
for key,items in cross.items():
    names=set(i['fieldname'] for i in items if i['fieldname'])
    doctypes=set(i['doctype'] for i in items)
    if len(items)>1 and len(names)>1:
        suggestion=canon_display_fieldname(sorted(names,key=lambda s:(len(s),s))[0])
        report['issues'].append({'type':'cross_doctype_similar_fieldnames','canonical_compare':key,'suggested_fieldname':suggestion,'count':len(items),'examples':items[:10]})
