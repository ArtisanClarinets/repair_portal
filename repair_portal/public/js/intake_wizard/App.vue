	...
	<div class="intake-wizard" role="application" aria-label="Intake wizard">
		<header class="wizard-header">
			<div class="wizard-title-block">
				...
				<p class="wizard-subtitle">
					Capture customer, instrument, player, and service insights with concierge-level
					guidance and auto-save safeguards.
				</p>
			...
			<aside class="wizard-progress" role="navigation" aria-label="Wizard progress">
				<div class="progress-header">
					...
					<span class="progress-steps"
						>Step {{ activeStepIndex + 1 }} of {{ steps.length }}</span
					>
				</div>
				...
				<ol class="step-list">
					...
					>
						...
						>
							...
							<span v-else-if="index === activeStepIndex" class="step-status"
								>In Progress</span
							>
				...
				<div class="progress-footer">
					...
					<button type="button" class="link-button" @click="openKnowledgeBase">
						Open Intake SOP
					</button>
				</div>
			</aside>
		...
		<footer class="wizard-footer">
			<div class="footer-support">
				...
				<button type="button" class="link-button" @click="startSupportChat">
					Message workshop lead
				</button>
			</div>
		...
		<transition name="summary-fade">
			<section v-if="submissionState.success" class="submission-summary" aria-live="polite">
				...
				<ul>
					<li>
						<a :href="submissionState.links.intake_form_route">Open Intake Record</a>
					</li>
					<li v-if="submissionState.links.intake_receipt_print">
						<a :href="submissionState.links.intake_receipt_print" target="_blank"
							>Print Intake Receipt</a
						>
					</li>
					<li v-if="submissionState.links.instrument_tag_print">
						<a :href="submissionState.links.instrument_tag_print" target="_blank"
							>Print Instrument Tag</a
						>
					</li>
					<li v-if="submissionState.links.instrument_qr_print">
						<a :href="submissionState.links.instrument_qr_print" target="_blank"
							>Print Instrument QR</a
						>
					</li>
					<li>
						<a
							:href="submissionState.links.create_repair_request_route"
							target="_blank"
		...
		<transition name="summary-fade">
			<section v-if="submissionState.error" class="submission-error" role="alert">
				...
				<p class="suggestion">
					Your progress is still saved. Resolve the error and try again.
				</p>
			</section>
		</transition>
	</div>
...
async function handlePlayerUpsert(data) {
	...
	try {
		...
		frappe.show_alert({
			message: `Player profile synced to ${linkedName}`,
			indicator: "green",
		});
		return response.message;
	} finally {
...
async function submitIntake() {
	...
	try {
		...
		const intakePayload = {
			...
			intake_type:
				service.intakeType || service.intakeDetails?.intake_type || "New Inventory",
			...
			acquisition_cost: Number.isFinite(service.acquisition_cost)
				? Number(service.acquisition_cost)
				: 0,
			store_asking_price: Number.isFinite(service.store_asking_price)
				? Number(service.store_asking_price)
				: 0,
		...
		if (
			service.loanerAgreement &&
			(service.loanerAgreement.borrower_signature || service.loanerAgreement.staff_signature)
		) {
...
async function loadSession(sessionId) {
	...
	try {
		...
		servicePayload.intakeDetails =
			rawService.intakeDetails || servicePayload.intakeDetails || {};
...
function startSupportChat() {
	frappe.show_alert({
		message: "Ping sent to workshop lead via desk notifications.",
		indicator: "blue",
	});
}
