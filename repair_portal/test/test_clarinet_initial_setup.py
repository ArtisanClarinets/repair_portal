...
class TestClarinetInitialSetup(unittest.TestCase):
    def setUp(self):
        if not frappe.db.exists("Customer", "test@example.com"):
            frappe.get_doc({"doctype": "Customer", "customer_name": "Test Customer", "email_id": "test@example.com"}).insert()
        if not frappe.db.exists("Item", "CLARINET01"):
            frappe.get_doc({"doctype": "Item", "item_code": "CLARINET01", "item_group": "Clarinets", "item_name": "Test Clarinet"}).insert()
