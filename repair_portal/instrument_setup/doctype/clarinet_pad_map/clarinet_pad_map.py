...
class ClarinetPadMap(Document):
    ...
    # ----- Detection -----
    def _get_category_title_from_links(self) -> str | None:
        """
    ...
    def _populate_soprano(self):
        for name in STANDARD_PAD_NAMES_TOP:
            self.append(
                ...
                {
                    "pad_position": name,
                    "is_open_key": 1 if name in OPEN_KEY_POSITIONS_SOPRANO else 0,
                },
            )
    ...
    def _populate_bass(self, variant: str | None):
    ...
    def _enforce_open_key_flags(self, family: str | None):
        """
        ...
        open_set = (
            OPEN_KEY_POSITIONS_SOPRANO
            if family == "soprano"
            else OPEN_KEY_POSITIONS_BASS if family == "bass" else OPEN_KEY_POSITIONS_SOPRANO
        )
            ...
            for row in rows or []:
                ...
                if current is None:
                    ...
                    try:
                        ...
                        frappe.log_error(
                            f"Invalid is_open_key value: {current}", "Clarinet Pad Map Data Error"
                        )
...
def populate_standard_pad_names(docname: str | None = None, doc_json: Any | None = None):
    """
