...
class SetupTemplate(Document):
    ...
    def on_update(self) -> None:
        ...
        """
        frappe.log(
            f'Setup Template updated: name={self.name}, workflow_state={self.workflow_state}'
        )
    ...
    def on_trash(self) -> None:
        ...
        """
        frappe.log(
            f'Setup Template deleted: name={self.name}, workflow_state={self.workflow_state}'
        )
        ...
        if frappe.db.exists('Instrument Profile', {'setup_template': self.name}):
            frappe.db.set_value(
                'Instrument Profile', {'setup_template': self.name}, 'setup_template', ''
            )
            frappe.msgprint(
                _("Setup Template '{0}' has been removed from related Instrument Profiles.").format(
                    self.name
                )
            )
    ...
    def on_load(self) -> None:
        ...
        if not self.checklist_items:
            ...
            frappe.msgprint(
                _("Checklist items for Setup Template '{0}' have been loaded.").format(self.name)
            )
        if not self.instrument_profile:
        ...
        if not self.workflow_state:
            ...
            frappe.msgprint(
                _("Workflow state for Setup Template '{0}' has been set to Draft.").format(
                    self.name
                )
            )
        ...
        if frappe.db.exists('Setup Checklist Item', {'parent': self.name}):
            ...
            frappe.msgprint(
                _("No checklist items found for Setup Template '{0}'.").format(self.name)
            )
        if frappe.db.exists('Instrument Profile', {'setup_template': self.name}):
            frappe.msgprint(
