...
class ClarinetInitialSetup(Document):
    def before_insert(self):
        if not self.technician:
            available = frappe.get_all('User', filters={"role_profile_name": "Technician"}, fields=['name'], limit=1)
            if available:
                self.technician = available[0].name

    def validate(self):
        for item in self.materials:
            bin_qty = frappe.db.get_value("Bin", {"item_code": item.item, "warehouse": item.warehouse}, "actual_qty") or 0
