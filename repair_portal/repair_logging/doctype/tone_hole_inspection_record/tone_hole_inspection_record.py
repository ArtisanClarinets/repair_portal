...
class ToneHoleInspectionRecord(Document):
        ...
        if self.tone_hole_number:
            ...
            if hole_num.isdigit():
                ...
                if hole_int < 1 or hole_int > 24:
                    frappe.msgprint(_("Warning: Tone hole number {0} is outside typical range (1-24)").format(hole_int))
    ...
    def _validate_inspection_criteria(self):
        ...
        if self.visual_status and self.visual_status not in valid_visual_status:
            frappe.throw(_("Invalid visual status: {0}. Valid options are: {1}").format(
    ...
    def _validate_inspector_permissions(self):
        ...
        if self.inspected_by:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.throw(_('User {0} does not have appropriate permissions for tone hole inspections.').format(self.inspected_by))
    ...
    def _validate_hole_numbering(self):
        ...
        if self.parent and self.parenttype:
            existing_holes = frappe.get_all(self.doctype,
                filters={
                    'parent': self.parent,
            ...
            if existing_holes:
                frappe.throw(_("Tone hole number {0} already exists in this inspection").format(self.tone_hole_number))
    ...
    def _validate_inspection_consistency(self):
        ...
        if self.visual_status and self.condition_rating:
            ...
            if self.visual_status in ['Excellent', 'Good'] and rating < 6:
                frappe.msgprint(_("Warning: Good visual status but low condition rating ({0})").format(rating))
            elif self.visual_status in ['Damaged', 'Cracked'] and rating > 4:
                frappe.msgprint(_("Warning: Damaged visual status but high condition rating ({0})").format(rating))
    ...
    def _check_maintenance_requirements(self):
        ...
        if self.visual_status in ['Damaged', 'Cracked', 'Chipped']:
            ...
            alerts.append(f"Tone hole {self.tone_hole_number}: {self.visual_status} - requires immediate attention")
        ...
        if self.condition_rating and flt(self.condition_rating) <= 3:
            maintenance_needed = True
            alerts.append(f"Tone hole {self.tone_hole_number}: Poor condition (rating {self.condition_rating})")
