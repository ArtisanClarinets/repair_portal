...
class RelatedInstrumentInteraction(Document):
    ...
    def _validate_instrument_references(self):
        ...
        if (
            ...
            and self.related_instrument
            and self.primary_instrument == self.related_instrument
        ):
    ...
    def _validate_relationship_type(self):
        ...
        if self.relationship_type and self.relationship_type not in valid_relationships:
            frappe.throw(
                _("Invalid relationship type: {0}. Valid options are: {1}").format(
    ...
    def _validate_user_permissions(self):
        ...
        if self.performed_by:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.msgprint(
                    _(
                        "Warning: User {0} may not have appropriate permissions for instrument interactions."
                    ).format(self.performed_by)
                )
    ...
    def _validate_cross_references(self):
        ...
        if self.primary_instrument and self.related_instrument and self.relationship_type:
            existing_relationship = frappe.db.exists(
                self.doctype,
                {
                    "name": ["!=", self.name],
                    "primary_instrument": self.primary_instrument,
            ...
            if existing_relationship:
                frappe.msgprint(
                    _("Warning: Similar relationship already exists: {0}").format(existing_relationship)
                )
    ...
    def _validate_relationship_consistency(self):
        ...
        if self.relationship_type == "Set Components":
            ...
            primary_model = frappe.db.get_value(
                "Instrument Profile", self.primary_instrument, "instrument_model"
            )
            related_model = frappe.db.get_value(
                "Instrument Profile", self.related_instrument, "instrument_model"
            )
    ...
    def _synchronize_repair_status(self):
        ...
        if self.relationship_type in ["Set Components", "Synchronized Repair"]:
            try:
                ...
                primary_status = frappe.db.get_value(
                    "Instrument Profile", self.primary_instrument, "repair_status"
                )
