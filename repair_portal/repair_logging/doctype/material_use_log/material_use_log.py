...
class MaterialUseLog(Document):
    ...
    def _validate_warehouse_permissions(self):
        ...
        if self.source_warehouse and not frappe.has_permission("Warehouse", "read", self.source_warehouse):
            frappe.throw(
                _("You do not have permission to access warehouse {0}").format(self.source_warehouse)
            )
    ...
    def _validate_operation_link(self):
        ...
        if (
            self.operation_type
            and self.operation_link
            and not frappe.db.exists(self.operation_type, self.operation_link)
        ):
            frappe.throw(
                _("Referenced {0} {1} does not exist.").format(self.operation_type, self.operation_link)
            )
    ...
    def _create_stock_movement(self):
        ...
        try:
            ...
            if self.source_warehouse:
                stock_qty = (
                    frappe.db.get_value(
                        "Bin", {"item_code": self.item_name, "warehouse": self.source_warehouse}, "actual_qty"
                    )
                    or 0
                )
                ...
                if stock_qty < self.qty:
                    frappe.msgprint(
                        _("Warning: Stock ({0}) is less than required quantity ({1}) for item {2}").format(
                            stock_qty, self.qty, self.item_name
                        )
                    )
