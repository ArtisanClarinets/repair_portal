...
class PadCondition(Document):
    ...
    def _validate_pad_identification(self):
        ...
        valid_locations = [
            'Upper Joint - Tone Hole 1', 'Upper Joint - Tone Hole 2', 'Upper Joint - Tone Hole 3',
            'Upper Joint - Tone Hole 4', 'Upper Joint - Tone Hole 5', 'Upper Joint - Tone Hole 6',
            'Lower Joint - Tone Hole 7', 'Lower Joint - Tone Hole 8', 'Lower Joint - Tone Hole 9',
            'Lower Joint - Tone Hole 10', 'Lower Joint - Tone Hole 11', 'Lower Joint - Tone Hole 12',
            'Bell - Tone Hole 13', 'Register Key', 'Thumb Hole', 'Other'
        ]
        ...
        if self.instrument_reference:
            ...
            instrument_type = frappe.db.get_value('Instrument Profile', self.instrument_reference, 'instrument_type')
    ...
    def _validate_condition_ratings(self):
        ...
        criteria_fields = [
            'sealing_effectiveness', 'pad_wear_level', 'spring_tension',
            'key_alignment', 'moisture_damage', 'surface_condition'
        ]
    ...
    def _validate_technician_qualifications(self):
        ...
        if self.assessed_by:
            ...
            technician_data = frappe.db.get_value('User', self.assessed_by, 
                ['enabled', 'full_name'])
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.msgprint(_('Warning: User {0} may not be certified for pad assessments.').format(self.assessed_by))
    ...
    def _update_instrument_pad_status(self):
        ...
        if self.instrument_reference:
            try:
                ...
                pad_conditions = frappe.get_all('Pad Condition',
                ...
                if pad_conditions:
                    ...
                    ratings = [flt(pad['condition_rating']) for pad in pad_conditions if pad['condition_rating']]
