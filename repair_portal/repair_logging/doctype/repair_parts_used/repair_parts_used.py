...
class RepairPartsUsed(Document):
    ...
    def _validate_part_exists(self):
        ...
        part_data = frappe.db.get_value(
            "Item", self.item_code, ["disabled", "is_stock_item", "item_name", "maintain_stock"]
        )
    ...
    def _validate_warehouse_permissions(self):
        ...
        if self.warehouse:
            ...
            warehouse_data = frappe.db.get_value("Warehouse", self.warehouse, ["disabled", "is_group"])
            ...
            if warehouse_data and warehouse_data[1]:  # is group
                frappe.throw(
                    _("Cannot use group warehouse {0}. Please select a specific warehouse.").format(
                        self.warehouse
                    )
                )
        ...
        try:
            ...
            if self.warehouse:
                ...
                if available_qty < flt(self.qty):
                    frappe.msgprint(
                        _(
                            "Warning: Available stock ({0}) is less than required quantity ({1}) for part {2}"
