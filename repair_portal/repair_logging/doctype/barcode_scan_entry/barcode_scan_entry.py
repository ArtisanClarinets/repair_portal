...
class BarcodeScanEntry(Document):
    ...
    def _resolve_barcode_to_item(self):
        ...
        if not self.linked_item:
            ...
            item_code = frappe.db.get_value("Item Barcode", {"barcode": self.barcode}, "parent")

            if item_code:
                # Validate item is active
                item_data = frappe.db.get_value("Item", item_code, ["disabled", "item_name"])
                ...
                if frappe.db.exists("Item", self.barcode):
                    item_data = frappe.db.get_value("Item", self.barcode, ["disabled", "item_name"])
        ...
        try:
            ...
            if self.linked_item:
                item_details = frappe.db.get_value(
                    "Item", self.linked_item, ["item_name", "item_group", "stock_uom"], as_dict=True
                )

                return {"success": True, "item_code": self.linked_item, "item_details": item_details}
            else:
