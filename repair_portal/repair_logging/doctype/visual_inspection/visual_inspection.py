...
class VisualInspection(Document):
    ...
    def _validate_inspection_criteria(self):
        ...
        if self.inspection_type and self.inspection_type not in valid_inspection_types:
            frappe.throw(
                _("Invalid inspection type: {0}. Valid options are: {1}").format(
        ...
        for field in rating_fields:
            if self.get(field):
                ...
                if rating < 1 or rating > 10:
                    frappe.throw(_("{0} must be between 1 and 10").format(field.replace("_", " ").title()))
    ...
    def _validate_inspector_permissions(self):
        ...
        if self.inspector:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.throw(
                    _("User {0} does not have appropriate permissions for visual inspections.").format(
                        self.inspector
                    )
                )
    ...
    def _validate_inspection_scope(self):
        ...
        if self.inspection_type in [
            "Pre-Repair Assessment",
            "Post-Repair Verification",
            "Quality Control",
        ] and not (self.body_condition_rating and self.key_condition_rating):
            frappe.msgprint(
                _("Warning: {0} typically requires detailed component ratings").format(self.inspection_type)
            )
            ...
            metadata.update(
                {
                    ...
                    "recommendations_count": (
                        len((self.recommendations or "").split("\n")) if self.recommendations else 0
                    ),
                }
            )
    ...
    def _validate_inspection_consistency(self):
        ...
        if self.overall_condition_rating and self.condition_status:
            ...
            if self.condition_status != expected_status:
                frappe.msgprint(
                    _("Warning: Condition status '{0}' may not match rating {1} (Expected: {2})").format(
        ...
        if self.overall_condition_rating:
            ...
            if valid_components:
                ...
                if abs(overall - avg_components) > 2:
                    frappe.msgprint(
                        _(
                            "Warning: Overall rating {0} differs significantly from component average {1}"
                        ).format(overall, round(avg_components, 1))
                    )
    ...
    def _validate_inspection_completion(self):
        ...
        if (
            self.condition_status in ["Poor", "Very Poor", "Requires Immediate Attention"]
    ...
    def _check_maintenance_alerts(self):
        ...
        if self.overall_condition_rating and flt(self.overall_condition_rating) <= 4:
            alerts.append(
                f"Overall condition rating {self.overall_condition_rating} indicates need for maintenance"
            )
    ...
    def calculate_condition_trends(self):
        ...
        recent_inspections = frappe.get_all(
            self.doctype,
            filters={
                "instrument_profile": self.instrument_profile,
                "docstatus": ["!=", 2],
        ...
        if len(ratings) >= 3:
            ...
            trend_direction = (
                ...
                else "declining" if ratings[0] < ratings[1] else "stable"
            )
