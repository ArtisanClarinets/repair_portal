...
class RepairTaskLog(Document):
    ...
    def _validate_technician_permissions(self):
        ...
        if self.technician:
            ...
            technician_data = frappe.db.get_value("User", self.technician, ["enabled", "full_name"])
    ...
    def _validate_time_tracking(self):
        ...
        if self.start_time and self.end_time:
            ...
            if duration > 24:
                frappe.throw(
                    _("Task duration ({0} hours) exceeds maximum allowed (24 hours).").format(duration)
                )
    ...
    def _validate_task_status(self):
        ...
        if self.status and self.status not in valid_statuses:
            frappe.throw(
                _("Invalid status: {0}. Valid options are: {1}").format(
    ...
    def _validate_billable_time(self):
        ...
        if self.billable_hours:
            ...
            if self.duration_hours and flt(self.billable_hours) > flt(self.duration_hours):
                frappe.throw(
                    _("Billable hours ({0}) cannot exceed total duration ({1}).").format(
        ...
        if self.parent and self.parenttype:
            try:
                ...
                completed_tasks = frappe.db.count(
                    self.doctype, {"parent": self.parent, "status": "Completed"}
                )
