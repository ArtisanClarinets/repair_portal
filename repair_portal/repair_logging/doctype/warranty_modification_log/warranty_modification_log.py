...
class WarrantyModificationLog(Document):
    ...
    def _validate_instrument_reference(self):
        ...
        if self.instrument_profile:
            ...
            instrument_data = frappe.db.get_value('Instrument Profile', self.instrument_profile, 
                ['instrument_type', 'serial_number', 'warranty_start_date', 'warranty_end_date'], as_dict=True)
    ...
    def _validate_modification_data(self):
        ...
        if self.modification_type and self.modification_type not in valid_modification_types:
            frappe.throw(_("Invalid modification type: {0}. Valid options are: {1}").format(
        ...
        if self.modification_reason:
            ...
            if self.modification_reason not in valid_reasons:
                frappe.msgprint(_("Warning: Non-standard modification reason: {0}").format(self.modification_reason))
    ...
    def _validate_authorization(self):
        ...
        if self.authorized_by:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.throw(_('User {0} does not have appropriate permissions to authorize warranty modifications.').format(self.authorized_by))
        
        # Validate approval requirements for significant modifications
        if (self.modification_type in ['Extension', 'Upgrade Coverage', 'Reinstatement'] and 
            not self.approval_reference):
            frappe.msgprint(_("Warning: Approval reference recommended for {0}").format(self.modification_type))
    ...
    def _validate_date_logic(self):
        ...
        if self.new_warranty_start_date and self.new_warranty_end_date:
            ...
            if warranty_days > 3650:  # 10 years
                ...
                frappe.msgprint(_("Warning: Warranty period {0} days is less than 30 days").format(warranty_days))
    ...
    def _validate_modification_authority(self):
        ...
        if self.instrument_profile and self.effective_date:
            conflicting_mods = frappe.get_all(self.doctype,
                filters={
                    'instrument_profile': self.instrument_profile,
            ...
            if conflicting_mods:
                frappe.msgprint(_("Warning: Conflicting modification on same date: {0}").format(
                    conflicting_mods[0].name))
    ...
    def _validate_modification_consistency(self):
        ...
        elif (self.modification_type == 'Cancellation' and 
              self.new_warranty_end_date and getdate(self.new_warranty_end_date) > getdate()):
    ...
    def _validate_modification_completion(self):
        ...
        if self.warranty_impact_days and abs(self.warranty_impact_days) > 365 and not self.detailed_justification:
    ...
    def _check_compliance_requirements(self):
        ...
        if (self.modification_type in ['Extension', 'Upgrade Coverage'] and 
            self.warranty_impact_days and self.warranty_impact_days > 730):  # 2 years
        ...
        if self.modification_type in ['Reduction', 'Cancellation']:
            compliance_alerts.append("Customer notification and consent required for warranty reduction/cancellation")
    ...
    def _generate_modification_certificate(self):
        ...
        certificate_data = {
            ...
            'new_warranty_period': f"{self.new_warranty_start_date} to {self.new_warranty_end_date}" if self.new_warranty_start_date and self.new_warranty_end_date else None,
    ...
    def _log_modification_audit(self):
        ...
        frappe.logger("warranty_modification_audit").info({
            ...
            "authorized_by": self.authorized_by,
            "effective_date": str(self.effective_date),
            "original_warranty_start": str(self.original_warranty_start) if self.original_warranty_start else None,
            "original_warranty_end": str(self.original_warranty_end) if self.original_warranty_end else None,
            "new_warranty_start": str(self.new_warranty_start_date) if self.new_warranty_start_date else None,
    ...
    def get_modification_history(self):
        ...
        history = frappe.get_all(self.doctype,
            ...
            fields=[
                'name', 'effective_date', 'modification_type', 'modification_reason',
                'authorized_by', 'warranty_impact_days', 'cost_impact'
            ],
    ...
    def calculate_warranty_timeline(self):
        ...
        modifications = frappe.get_all(self.doctype,
            filters={
                'instrument_profile': self.instrument_profile,
                'docstatus': 1  # Only submitted modifications
            },
            fields=[
                'effective_date', 'modification_type', 'warranty_impact_days',
                'new_warranty_start_date', 'new_warranty_end_date'
            ],
