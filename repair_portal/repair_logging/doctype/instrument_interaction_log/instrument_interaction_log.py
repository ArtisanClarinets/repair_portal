...
class InstrumentInteractionLog(Document):
    ...
    def _validate_user_permissions(self):
        ...
        if self.interaction_type in restricted_interactions:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.throw(_("Insufficient permissions for {0} interaction type").format(self.interaction_type))

    def _validate_interaction_type(self):
        """Validate interaction type is valid."""
        valid_types = [
            'Inspection', 'Maintenance', 'Repair', 'Calibration', 
            'Testing', 'Diagnostic', 'Cleaning', 'Setup', 
            'Documentation', 'Other'
        ]
        
        if self.interaction_type and self.interaction_type not in valid_types:
            frappe.throw(_("Invalid interaction type: {0}. Valid options are: {1}").format(
    ...
    def _validate_interaction_data(self):
        ...
        if self.duration_minutes and (float(self.duration_minutes) < 0 or float(self.duration_minutes) > 1440):
        ...
        if self.outcome and self.outcome not in valid_outcomes:
            frappe.throw(_("Invalid outcome: {0}. Valid options are: {1}").format(
    ...
    def _validate_data_consistency(self):
        ...
        if self.interaction_type and self.instrument_id and self.interaction_timestamp:
            ...
            existing = frappe.db.exists(self.doctype, {
                'name': ['!=', self.name],
    ...
    def get_interaction_history(self):
        ...
        interactions = frappe.get_all(self.doctype, 
