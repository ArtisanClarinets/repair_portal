...
class DiagnosticMetrics(Document):
    ...
    def _validate_technician_qualifications(self):
        ...
        if self.measured_by:
            ...
            if not frappe.db.exists('Has Role', {'parent': self.measured_by, 'role': ['in', ['Technician', 'Diagnostic Specialist']]}):
                frappe.msgprint(_('Warning: User {0} may not be certified for diagnostic measurements.').format(self.measured_by))
    ...
    def _update_metric_calculations(self):
        ...
        self.requires_attention = self.is_out_of_range() or (self.diagnostic_score and self.diagnostic_score < 70)
