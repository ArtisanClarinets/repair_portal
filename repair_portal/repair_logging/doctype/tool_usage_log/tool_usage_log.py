...
class ToolUsageLog(Document):
    ...
    def _validate_tool_reference(self):
        ...
        if self.tool:
            ...
            tool_data = frappe.db.get_value('Tool', self.tool, 
                ['tool_name', 'tool_type', 'status', 'calibration_due_date'], as_dict=True)
            ...
            if tool_data:
                ...
                if tool_data.status not in ['Available', 'In Use']:
                    frappe.throw(_("Tool {0} is not available for use (Status: {1})").format(
    ...
    def _validate_user_permissions(self):
        ...
        if self.used_by:
            ...
            if not any(role in user_roles for role in required_roles):
                frappe.throw(_('User {0} does not have appropriate permissions for tool usage.').format(self.used_by))
    ...
    def _validate_calibration_status(self):
        ...
        tool_data = frappe.db.get_value('Tool', self.tool, 
            ['requires_calibration', 'calibration_due_date', 'tool_type'], as_dict=True)
        ...
        if tool_data and tool_data.requires_calibration:
            if tool_data.calibration_due_date:
                ...
                if due_date < today:
                    frappe.throw(_("Tool {0} calibration is overdue (Due: {1})").format(
                        self.tool, due_date))
                elif (due_date - today).days <= 7:
                    frappe.msgprint(_("Warning: Tool {0} calibration due soon ({1})").format(
                        self.tool, due_date))
            else:
                frappe.msgprint(_("Warning: Tool {0} requires calibration but no due date set").format(self.tool))
    ...
    def _normalize_usage_data(self):
        ...
        if self.usage_type and self.usage_type not in valid_usage_types:
            frappe.throw(_("Invalid usage type: {0}. Valid options are: {1}").format(
    ...
    def _validate_tool_availability(self):
        ...
        concurrent_usage = frappe.get_all('Tool Usage Log',
            filters={
                'tool': self.tool,
                'docstatus': ['!=', 2],
                'end_time': ['is', 'not set'],
        ...
        if concurrent_usage:
            ...
            frappe.throw(_("Tool {0} is currently in use by {1} (Started: {2})").format(
    ...
    def _validate_usage_consistency(self):
        ...
        if self.actual_duration:
            ...
            if duration_minutes > 480:  # 8 hours
                frappe.msgprint(_("Warning: Usage duration {0} minutes exceeds 8 hours").format(duration_minutes))
            elif duration_minutes < 1:
                frappe.msgprint(_("Warning: Usage duration {0} minutes is very short").format(duration_minutes))
    ...
    def _check_maintenance_requirements(self):
        ...
        try:
            tool_data = frappe.db.get_value('Tool', self.tool, 
                ['maintenance_schedule', 'total_usage_hours', 'next_maintenance_hours'], as_dict=True)
            ...
            if tool_data and tool_data.next_maintenance_hours:
                ...
                if current_hours >= maintenance_due_hours:
                    frappe.msgprint(_("Tool {0} requires scheduled maintenance (Usage: {1} hours)").format(
                        self.tool, current_hours), alert=True)
    ...
    def _update_tool_lifecycle(self):
        ...
        try:
            ...
            frappe.logger("tool_lifecycle").info({
                ...
                "lifecycle_data": lifecycle_record
            })
    ...
    def get_tool_usage_history(self):
        ...
        history = frappe.get_all(self.doctype,
            ...
            fields=[
                'name', 'start_time', 'end_time', 'usage_type', 
                'used_by', 'actual_duration', 'usage_notes'
            ],
