...
function render_timeline_with_filter(frm) {
  const container = $(frm.fields_dict.interaction_logs.wrapper).parent().prepend(`
...
function add_export_button(frm) {
  frm.add_custom_button('Export Log as CSV', () => {
    const rows = [
      ['Interaction Type', 'Reference DocType', 'Reference Name', 'Date', 'Notes']
    ];
    (frm.doc.interaction_logs || []).forEach(log => {
      rows.push([
    ...
    link.setAttribute("download", `Instrument_Log_${frm.doc.serial_number || 'tracker'}.csv`);
