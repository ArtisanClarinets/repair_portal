...
# Dependencies: Frappe, Player Profile, Intonation Session, Instrument Profile, Clarinet Inspection
...
def get_context(context):
    ...
    qa_pass_count = frappe.db.count(
        'Clarinet Inspection', {'player_profile': profile.name, 'status': 'Pass'}
    )
    context.analytics = {
    ...
    if dob:
        ...
        if age < 13:
            ...
            context.compliance['warning'] = _(
                'Marketing is blocked for this player due to age/compliance.'
            )
    else:
    ...
    if not frappe.session.user == parent_user and frappe.session.user != profile.owner:
