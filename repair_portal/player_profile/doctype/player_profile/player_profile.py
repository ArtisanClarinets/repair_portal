...
def _resolve_serial_display(instrument_profile_row: dict) -> Optional[str]:
    """
...
class PlayerProfile(Document):
    ...
    if TYPE_CHECKING:
        ...
        player_level: DF.Literal["Student (Beginner)", "Student (Advanced)", "Amateur/Hobbyist", "University Student", "Professional (Orchestral)", "Professional (Jazz/Commercial)", "Educator", "Collector"] #type: ignore
        player_name: DF.Data
    ...
    def _block_marketing_emails(self):
        ...
        try:
            ...
            customer = frappe.db.get_value("Customer", {"email_id": self.primary_email}, ["linked_user"], as_dict=True) #type: ignore
    ...
    def _sync_instruments_owned(self):
        ...
        try:
            owned = frappe.get_all(
                ...
                fields=["name", "serial_no", "model", "instrument"]  # include instrument link for ISN-aware resolution
            )
