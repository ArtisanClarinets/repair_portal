frappe.ui.form.on('Repair Request', {
  refresh: function(frm) {
    ...
    if (frm.doc.repair_status && status_color_map[frm.doc.repair_status]) {
      frm.dashboard.set_headline(`<span style='color:${status_color_map[frm.doc.repair_status]}'>Status: ${frm.doc.repair_status}</span>`);
    }
  },
  ...
  requested_services: function(frm) {
    ...
    frm.set_df_property('total_estimated_price', 'description', `Estimated Repair Cost: $${total.toFixed(2)}`);
  }
});