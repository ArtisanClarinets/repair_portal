    ...
    def after_insert(self):
        ...
        try:
            ...
            if serial_no_input:
                ...
                if not instrument:
                    ...
                    if self.instrument_category and frappe.db.exists(
                        ...
                        default_cat = frappe.db.get_value(
            ...
            if not frappe.db.exists('Instrument Inspection', {'intake_record_id': self.name}):
                ...
                inspected_by_meta = frappe.get_meta('Instrument Inspection').get_field(
                    'inspected_by'
                )
                inspected_by_options = (
                    (inspected_by_meta.options or 'User') if inspected_by_meta else 'User'
                )
    ...
    def _sync_info_from_existing_instrument(self) -> None:
        ...
        if self.serial_no and not self.instrument:  # type: ignore
            ...
            if isn and isn.get('name'):
                ...
                field_type = _get_instrument_serial_field_type()
                if field_type == 'Link':
                    instr_name = frappe.db.get_value(
                        'Instrument', {'serial_no': isn['name']}, 'name'
                    )
                    if instr_name:
            ...
            if not instr_doc:
                ...
                instr_name = frappe.db.get_value(
                    'Instrument', {'serial_no': serial_no_input}, 'name'
                )
                if instr_name:
                    instr_doc = frappe.get_doc('Instrument', instr_name)  # type: ignore
