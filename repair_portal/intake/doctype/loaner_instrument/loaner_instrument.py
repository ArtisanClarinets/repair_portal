...
class LoanerInstrument(Document):
    ...
    def _validate_business_rules(self) -> None:
        ...
        if self.status == "Returned" and self.flags.in_insert == False:
            ...
            previous = (
                frappe.db.get_value(self.doctype, self.name, ["docstatus", "status"], as_dict=True)
                if self.name
                else None
            )
        ...
        if self.status == "Returned" and not self.condition_on_return:
            frappe.msgprint(_("Consider recording 'Condition on Return' for audit."), indicator="yellow")
    ...
    def _build_template_context(self) -> dict:
        ...
        return {
            ...
            "company_full_name": frappe.db.get_value(
                "Company", frappe.defaults.get_global_default("company"), "company_name"
            ),
        }
