...
# Version: v1.1
# Purpose: Load all JSON setup template files from the intake/hooks/templates directory and insert/update them in Frappe.
    ...
    try:
        ...
        for file_name in os.listdir(base_path):
            ...
            try:
                ...
                if frappe.db.exists("Item Group", template_name):
                    # Update existing template
                    doc = frappe.get_doc("Item Group", template_name)
                ...
                frappe.log_error(f"Failed to insert/update {template_name}: {str(e)}", "Setup Template Loader")

        frappe.db.commit()

    except Exception:
        frappe.log_error(frappe.get_traceback(), "Intake Loader Failed")