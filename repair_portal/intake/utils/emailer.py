...
def queue_intake_status_email(intake_name: str, event: str = "state_change") -> None:
    ...
    if not getattr(intake, "customer_email", None):
        LOGGER.warning(
            "Intake %s has no customer_email; skipping notification", intake_name
        )
        return
...
def _build_message(event: str, intake) -> str:
    ...
    if event == "submitted":
        ...
        lines.append(
            _(
                "<p>Your instrument’s status has been updated to "
                "<strong>{0}</strong>.</p>"
            ).format(intake.workflow_state or _("Unknown"))
        )

    lines.append(
        _("<p><a href='{0}'>View your Intake record</a></p>").format(intake_url)
    )
