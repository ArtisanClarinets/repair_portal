    ...
    for doc in frappe.get_all("Inspection Report", fields=["name"]):
        ...
        try:
            ...
            exists = frappe.db.exists(
                "Intake Inspection",
                {"clarinet_intake": old.clarinet_intake, "inspection_date": old.inspection_date},
            )
            if exists:
                continue
            ...
            for field in [
                ...
                "clarinet_intake",
                "legacy_clarinet_inspection_id",
                "qc_certificate",
                "non_conformance_report",
                "digital_signature",
                "flag_for_reinspection",
            ]:
            ...
            frappe.log_error(
                f"Failed to migrate Inspection Report {doc.name}: {e}",
                "Inspection Report Migration Error",
            )
