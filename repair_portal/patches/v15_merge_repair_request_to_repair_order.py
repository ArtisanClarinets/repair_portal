...
def execute():
    ...
    for req in req_list:
        ...
        notes = frappe.get_all(
            "Repair Note",
            filters={"parent": req.name, "parenttype": "Repair Request"},
            fields=["*"],
        )
        for note in notes:
            note.pop("name", None)
        ...
        count += 1
    frappe.msgprint(
        f"{count} Repair Requests migrated to Repair Order. Review status and close old requests when confirmed."
    )
