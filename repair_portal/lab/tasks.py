...
def _attach_png(fig_or_bytes, filename: str, attach_to_doctype: str, attach_to_name: str) -> str:
    ...
    if plt is not None and hasattr(fig_or_bytes, "savefig"):
        ...
        # already bytes
        content = fig_or_bytes if isinstance(fig_or_bytes, (bytes, bytearray)) else bytes()
...
def _analyze_intonation(sess, row, raw, a4: float):
    ...
    points = [{"label": name, "group": _group_register(name), "x": 0.0, "y": float(avg), "z": float(std)} for (name, avg, std) in per_note]
    ...
    if plt is not None:
        ...
        ts_pairs = [(float(fr.get("t") or 0), float(fr.get("f0") or 0)) for fr in frames if fr.get("t") is not None]
        if ts_pairs:
            ts_pairs.sort()
            if np is not None:
...
def _analyze_resonance(sess, row, raw, a4: float):
    ...
    points = [{"label": f"Peak {i+1}", "group": "ESS-peaks", "x": float(f), "y": float(m)} for i, (f, m) in enumerate(filtered)]
    ...
    if plt is not None:
        ...
        ax.set_xlabel("Hz"); ax.set_ylabel("|H(f)| (dB)")
...
def _analyze_leak(sess, row, raw):
    ...
    points = [{"label": str(h.get("label") or ""), "group": "leak", "x": 0.0, "y": float(h.get("score") or 0.0)} for h in holes]
    ...
    if plt is not None:
        ...
        ax.bar(range(len(vals)), vals)
        ax.set_xticks(range(len(vals))); ax.set_xticklabels(labels, rotation=90)
        ax.set_ylim(0, 1)
...
def _analyze_measurement(sess, row, raw):
    ...
    points = [{"label": str(p.get("label") or ""), "group": "measurement", "x": 0.0, "y": float(p.get("value") or 0.0), "meta_json": json.dumps({"unit": p.get("unit")})} for p in params]
...
def _update_rollups(sess_doc):
    ...
    ints = []; ress = []; leaks = []; tones = []
    for t in sess_doc.tests or []:
        m = _parse(t.metrics_json) or {}
        if "intonation_score" in m: ints.append(float(m["intonation_score"]))
        if "resonance_score" in m: ress.append(float(m["resonance_score"]))
        if "leak_score" in m: leaks.append(float(m["leak_score"]))
        if "tone_score" in m: tones.append(float(m["tone_score"]))
    ...
    parts = []
    if sess_doc.intonation_score is not None: parts.append(sess_doc.intonation_score * weights["intonation"])
    if sess_doc.resonance_score is not None: parts.append(sess_doc.resonance_score * weights["resonance"])
    if sess_doc.leak_score is not None: parts.append(sess_doc.leak_score * weights["leak"])
    if sess_doc.tone_score is not None: parts.append(sess_doc.tone_score * weights["tone"])
...
def _update_instrument_wellness(sess_doc):
    ...
    trend.append({
        "ts": frappe.utils.now(),
        "session": sess_doc.name,
        "overall": iws.overall_score
    })
    # keep last 50
