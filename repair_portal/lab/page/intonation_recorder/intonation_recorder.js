frappe.pages['intonation_recorder'].on_page_load = function(wrapper) {
    ...
    let mediaStream, audioCtx, analyser, recorderChunks = [];
    let running = false, lastPitch = null, peaks = [];

    async function start_recording() {
        if (running) { stop_recording(); return; }
        ...
        async function stop_recording() {
            ...
            const safe_filename = 'intonation_' + frappe.datetime.now_datetime().replace(/[: ]/g,'_') + '.webm';

            frappe.call('repair_portal.lab.api.save_intonation_session', {
                instrument: null,
                player: null,
