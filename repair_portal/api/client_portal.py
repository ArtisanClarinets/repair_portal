...
def get_my_instruments():
    ...
    client = frappe.db.get_value("Client Profile", {"linked_user": frappe.session.user}, "name")
    if not client:
        return []

    player_names = frappe.get_all("Player Profile", {"client_profile": client}, pluck="name")
