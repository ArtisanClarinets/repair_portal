...
def validate_json_file(file_path: Path) -> list[str]:
    ...
    for fld, expected in mandatory_values.items():
        if fld in data:
            ...
            if str(actual).lower() != str(expected).lower():
                errors.append(
                    f"❌ {file_path}: Invalid value for '{fld}'. " f'Expected "{expected}", found "{actual}".'
                )
    ...
    if doc_type == "DocType" and data.get("istable") == 1:
        ...
        missing_istable_fields = [fld for fld in istable_required_fields if fld not in doc_type_fields]
