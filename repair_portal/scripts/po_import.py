...
def parse_invoice(txt: str, filename: str) -> Optional[InvoiceDoc]:
    m = _HDR.search(txt)
    if not m:
...
def create_po(frappe, cfg: Config, doc: InvoiceDoc) -> Optional[str]:
...
def main():
    ...
    parser.add_argument("--expense-acct", dest="expense_account", default="Cost of Goods Sold - MAI")
