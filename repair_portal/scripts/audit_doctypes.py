...
RESOURCE_TYPES_WITH_EXTRA = {
    "DocType", "Report", "Dashboard", "Dashboard Chart", "Number Card", "Workspace"
}
...
class DocInfo(Resource):
    ...
    def __init__(self, name: str, module: str, path: Path, data: Dict):
        ...
        self.fieldnames: List[str] = [
            f.get("fieldname").strip() for f in self.fields if isinstance(f, dict) and f.get("fieldname")
        ]
...
def run():
    ...
    doctypes: Dict[str, DocInfo] = {name: r for name, r in catalog_by_type.get("DocType", {}).items() if isinstance(r, DocInfo)}
    ...
    if nd_warnings:
        print(f"⚠️ Potential near-duplicates by field overlap (DocType, threshold ≥ {NEAR_DUPLICATE_THRESHOLD}):")
    ...
    cross_refs: List[Tuple[str, str, str, str]] = []  # (parent_doctype, parent_module, fieldtype, child_doctype)
