...
def _parse_blocks(lines: List[str]) -> List[List[str]]:
    ...
    for ln in lines:
        low = ln.strip().lower()
        if re2.search(r"page\s*\d+\s*of\s*\d+", low): continue
        if _HDR.search(ln): continue
...
def parse_invoice(txt: str, name: str) -> Optional[InvoiceDoc]:
    m = _HDR.search(txt)
    if not m:
...
def load_invoices(dir: Path, show=False, one: Optional[str]=None) -> List[InvoiceDoc]:
...
def _pr(f, cfg: Config, doc: InvoiceDoc, seen: Set[str]) -> Optional[str]:
...
def _pi(f, cfg: Config, pr_name: str, doc: InvoiceDoc) -> Optional[str]:
    ...
    for row in pr.items:
        pi.append("items",{
            **{k:row.get(k) for k in (
                ...
                "discount_percentage","discount_amount","custom_origin_country","custom_hts_code"
            )},
