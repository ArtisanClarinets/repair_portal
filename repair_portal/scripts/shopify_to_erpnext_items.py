...
def convert(shopify_csv: Path, item_template_csv: Path, output_csv: Path):
    ...
    for s in shop_rows:
        ...
        # ─ basic mappings ─ #
        if COL["code"]  : row[COL["code" ]]  = s["Variant SKU"]
        if COL["name"]  : row[COL["name" ]]  = s["Title"]
        if COL["group"] : row[COL["group"]]  = s["Type"]
        if COL["desc"]  : row[COL["desc" ]]  = s["Body (HTML)"]

        # ─ constants & flags ─ #
        if COL["uom"]        : row[COL["uom"]]        = "Nos"
        if COL["stock_flag"] : row[COL["stock_flag"]] = 1

        # ─ inventory & pricing ─ #
        if COL["opening"]   : row[COL["opening"]]   = safe_float(s.get("Variant Inventory Qty", 0))
        if COL["sell_rate"] : row[COL["sell_rate"]] = safe_float(s.get("Variant Price", 0))
        if COL["cost_rate"] : row[COL["cost_rate"]] = safe_float(s.get("Cost per item", 0))
        ...
        if "Disabled" in row:        row["Disabled"]        = 0
        if "Has Variants" in row:    row["Has Variants"]    = 0
        if "Is Fixed Asset" in row:  row["Is Fixed Asset"]  = 0
