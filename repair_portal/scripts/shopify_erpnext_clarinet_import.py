...
def main():
    ...
    try:
        ...
        print(f"   ❌ FAILED: Could not reach the server.", file=sys.stderr)
        print(f"      Error: {e}", file=sys.stderr)
        print(f"      Please check the ERPNEXT_URL and your network connection.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"   ❌ FAILED: Could not authenticate.", file=sys.stderr)
        print(f"      Error: {e}", file=sys.stderr)
        print(f"      This usually means the API Key or Secret is incorrect.", file=sys.stderr)
        sys.exit(1)
