        ...
        if autoname:
            ...
            if autoname.startswith('format:'):
                ...
                try:
                    ...
                    for fld in fields:
                        if fld not in fieldnames and fld not in {'YY', 'YYYY', 'MM', 'DD'}:
                            warnings.append(
                                f"format placeholder '{{{fld}}}' not a field in DocType"
                            )
                ...
                try:
                    ...
                    if not df:
                        ...
                        if not getattr(df, 'unique', 0):
                            warnings.append(
                                f"field '{fld}' is not Unique (duplicate saves will fail)"
                            )
                except Exception as e:
                    warnings.append(f'meta lookup failed: {e}')
                ...
                try:
                    meta = frappe.get_meta(doctype)
                    ns_field = next(
                        (f for f in meta.fields if f.fieldname == 'naming_series'), None
                    )
                    if not ns_field:
                        warnings.append(
                            "uses naming_series but DocType lacks a 'naming_series' field"
                        )
                except Exception as e:
                    warnings.append(f'meta lookup failed: {e}')
                ...
                warnings.append(
                    "legacy/implicit pattern; prefer 'format:' (e.g., format:OPRT-{#####})"
                )
