...
SEARCH_GLOB = os.path.join(APP_ROOT, "**", "doctype", "*", "*.json")  # include workflow json too; we'll filter
...
def _audit_doc(json_path: str) -> Tuple[str, str, List[str]]:
    ...
    if not autoname and not has_controller_autoname:
        ...
        if has_controller_autoname and autoname:
            warnings.append("JSON 'autoname' present AND controller 'autoname' defined — controller takes precedence")
