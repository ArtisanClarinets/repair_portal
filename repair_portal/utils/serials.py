...
try:
    ...
    from repair_portal.instrument_profile.doctype.instrument_serial_number.instrument_serial_number import (  # type: ignore
...
if not _HAVE_CONTROLLER_NORMALIZE:
    ...
    def _controller_normalize_serial(s: Optional[str]) -> Optional[str]:
        if not s:
            return None
        s = s.strip().upper()
...
def normalize_serial(s: Optional[str]) -> Optional[str]:
...
def find_by_serial(serial_input: str) -> Optional[Dict[str, Any]]:
    """
    ...
    if not name:
        return None
    return frappe.db.get_value("Instrument Serial Number", name, "*", as_dict=True)


def find_by_scan_code(scan_code: str) -> Optional[Dict[str, Any]]:
...
def ensure_from_document(
    *,
    ...
    scan_code_field: Optional[str] = None,
    status: str = "Active",
    serial_source: Optional[str] = None,
    verification_status: Optional[str] = None,
    link_on_instrument: bool = True,
...
def merge_serials(
    *,
    ...
    if mark_duplicate_of:
        ...
        if getattr(dup, "status", None) and dup.status != "Deprecated":
