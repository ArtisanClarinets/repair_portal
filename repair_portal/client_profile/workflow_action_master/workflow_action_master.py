# repair_portal/client_profile/workflow_action_master/workflow_action_master.py
#
# Version: 2.0.0
#
# Purpose: Master controller for all Client Profile workflow transitions.
# This script is designed to be the single source of truth for what should
# happen when a Client Profile document transitions from one state to another.
#
# ---------------------------------------------------------------------------
...
def handle_workflow_action(doc: Document, action: str):
    """
    This function is called from the Client Profile DocType's `after_workflow_action` hook.
...
def handle_activation(doc: Document):
    ...
    if not frappe.db.exists("Player Profile", {"client_profile": doc.name}):
        frappe.get_doc(
            {
                "doctype": "Player Profile",
                "client_profile": doc.name,
...
def handle_approval(doc: Document):
    ...
    frappe.msgprint("Client Profile approved.")
...
def archive_children(doc: Document):
    ...
    """
    players = frappe.get_all("Player Profile", {"client_profile": doc.name})
    for p in players:
