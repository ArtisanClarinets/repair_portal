...
class ConsentAutofillMapping(Document):
    ...
    def _validate_mapping_configuration(self) -> None:
        ...
        if self.variable_name:
            ...
            if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", self.variable_name):
                frappe.throw(
                    _("Variable Name must be a valid Python identifier: {0}").format(self.variable_name)
                )
    ...
    def _validate_field_compatibility(self) -> None:
        ...
        if not source_field:
            frappe.throw(
                _("Source field '{0}' does not exist in DocType '{1}'").format(
    ...
    def _validate_circular_references(self) -> None:
        ...
        try:
            ...
            if not link_field:
                frappe.throw(
                    _("Form Link Field '{0}' does not exist in {1}").format(
            ...
            if link_field.options != self.source_doctype:
                frappe.throw(
                    _("Form Link Field '{0}' points to '{1}', but Source DocType is '{2}'").format(
    ...
    def get_mapped_value(self, source_doc_name: str | None = None) -> Any:
        ...
        try:
            ...
            value = frappe.db.get_value(self.source_doctype, source_doc_name, self.source_fieldname)
    ...
    def test_mapping(self, test_doc_name: str | None = None) -> dict[str, Any]:
        ...
        result = {"success": False, "value": None, "error": None, "warnings": []}

        try:
            if not test_doc_name and self.source_doctype:
                # Get a sample document for testing
                sample_docs = frappe.get_all(self.source_doctype, limit=1, fields=["name"])
                if sample_docs:
    ...
    def get_active_mappings(parent_doctype: str, parent_name: str) -> list[ConsentAutofillMapping]:
        ...
        return frappe.get_all(
            ...
            filters={"parent": parent_name, "parenttype": parent_doctype, "enabled": 1},
            fields=["*"],
    ...
    def apply_mappings(target_doc: Document, mappings: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        if not mappings:
            mappings = ConsentAutofillMapping.get_active_mappings(target_doc.doctype, target_doc.name)

        results = {"applied_count": 0, "failed_count": 0, "warnings": [], "details": []}
