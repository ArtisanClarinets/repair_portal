...
class ConsentTemplate(Document):
    ...
    def get_usage_statistics(self) -> dict[str, Any]:
        ...
        if frappe.db.exists("DocType", "Consent Form"):
            ...
            stats["total_forms"] = frappe.db.count("Consent Form", {
                "consent_template": self.name
            })
            
            # Active (draft) forms
            stats["active_forms"] = frappe.db.count("Consent Form", {
                "consent_template": self.name,
                "docstatus": 0
            })
            
            # Submitted forms
            stats["submitted_forms"] = frappe.db.count("Consent Form", {
                "consent_template": self.name,
                "docstatus": 1
            })
            
            # Recent forms (last 10)
            stats["recent_forms"] = frappe.db.get_all("Consent Form", {
                "consent_template": self.name
    ...
    def _validate_required_fields_structure(self) -> None:
        ...
        for field in self.required_fields:
            ...
            if field.field_type not in ["Data", "Text", "Select", "Check", "Date", "Datetime", "Int", "Float"]:
    ...
    def _has_active_forms(self) -> bool:
        ...
        return frappe.db.exists("Consent Form", {
            "consent_template": self.name,
            "docstatus": 0
        })
    ...
    def _update_related_forms(self) -> None:
        ...
        forms = frappe.db.get_all("Consent Form", {
            "consent_template": self.name,
            "docstatus": 0  # Only update draft forms
        }, ["name"])
