...
class LinkedPlayers(Document):
    ...
    def _enforce_single_primary(self) -> None:
        ...
        primaries = [
            d for d in siblings 
            if cint(d.is_primary) and d.name != self.name
        ]
    ...
    def _sync_player_details(self) -> None:
        ...
        if self.player_profile:
            player_details = frappe.db.get_value("Player Profile", 
                self.player_profile,
        ...
        if self.person:
            person_details = frappe.db.get_value("Person", 
                self.person,
    ...
    def _log_relationship_change(self) -> None:
        ...
        if self.has_value_changed("is_primary") and self.is_primary:
            frappe.logger().info(
                f"Player {self.player_profile} set as primary for customer {self.parent}"
            )
    ...
    def as_dict_safe(self) -> dict[str, any]:
        ...
        public_fields = {
            'person', 'person_name', 'player_profile', 'player_name',
            'relationship', 'date_linked', 'is_primary', 'notes',
            'instrument_category', 'skill_level', 'contact_email', 'contact_mobile'
        }
    ...
    def get_available_players(customer: str) -> list[dict]:
        ...
        linked_players = frappe.db.get_list("Linked Players",
        ...
        return frappe.db.get_list("Player Profile",
            filters=filters,
