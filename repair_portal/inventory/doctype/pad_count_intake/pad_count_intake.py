...
# Version: v1.5.0 (No-ML Pro Mode + PDF kit + pre-save companion API)
...
def generate_shooting_kit(name: str, marker_mm: float | None = None, aruco_dict: str | None = None):
    """Create & attach a multi-page Shooting Kit PDF + an instruction JPG (sets Photo)."""
    ...
    try:
        ...
        doc._append_log("generate_shooting_kit", f"pdf={pdf_file.file_url}, img={jpg_file.file_url}") # type: ignore
        doc.save()
        frappe.db.commit()
    except Exception:
    ...
    return {"photo": doc.photo, "pdf_url": pdf_file.file_url, "message": _("Shooting kit generated and attached.")} # type: ignore
...
def process_image(name: str):
    ...
    return {"detected_count": detected_count, "processed_preview": doc.processed_preview, "detections_meta": doc.detections_meta} # type: ignore
...
def update_inventory(name: str):
    """Create/submit Stock Entry based on inventory_action and inventory_delta."""
    doc = frappe.get_doc("Pad Count Intake", name)
    ...
    purpose = "Material Receipt" if doc.inventory_action == "Increase" and delta > 0 else "Material Issue" # type: ignore
    qty = abs(delta)
...
# ---------- Detection (No-ML: ArUco + Multi-pass Fusion) ----------
...
def detect_pads(img_bytes: bytes, params: dict):
    ...
    if not HAS_CV2:
        ...
        gray = im.convert("L").filter(ImageFilter.GaussianBlur(radius=max(1, (params.get("blur") or 7)//2)))
...
def _detect_hough(gray, params) -> List[Detection]:
    ...
    circles = cv2.HoughCircles(
        gray, cv2.HOUGH_GRADIENT, dp=float(params["dp"]), minDist=minDist,
...
def _detect_contours(gray, params) -> List[Detection]:
    ...
    for c in cnts:
        ...
        if circularity < 0.6:
            continue
        dets.append(Detection(x=int(x), y=int(y), r=int(r), conf=float(min(1.0, circularity)), method="contour"))
    return dets
...
def _detect_template(gray, params) -> List[Detection]:
    ...
    edges = cv2.Canny(gray, threshold1=max(20, int(params["param1"] * 0.5)), threshold2=int(params["param1"]))
    ...
    for (y, x) in zip(ys.tolist(), xs.tolist()):
        ...
        cx = x + size // 2
        cy = y + size // 2
        dets.append(Detection(x=int(cx), y=int(cy), r=int(r), conf=float(min(1.0, res[y, x])), method="template")) # type: ignore
...
def _annotate(img_bgr, dets: List[Detection]):
    ...
    cv2.putText(out, f"Detected: {len(dets)}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def _try_aruco_rectify_and_scale(img_bgr, aruco_dict_name: str, marker_mm: float):
    ...
    for i, c in enumerate(corners):
        areas.append(cv2.contourArea(c[0].astype(np.float32)))
...
def _build_instruction_image(marker_png: bytes, marker_mm: float, aruco_dict: str) -> bytes:
    ...
    cv2.putText(img, "Shooting Instructions", (30, 75), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (255, 255, 255), 3, cv2.LINE_AA)
    ...
    cv2.putText(img, f"ArUco: {aruco_dict}", (40, H - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (80, 80, 80), 2, cv2.LINE_AA)
