...
frappe.ui.form.on("Pad Count Intake", {
	refresh(frm) {
						...
						frappe
							...
							.then((r) => {
								...
								if (r.message && r.message.stock_entry) {
									frappe.msgprint(
										`Stock Entry: <a href="/app/stock-entry/${r.message.stock_entry}">${r.message.stock_entry}</a>`,
									);
								}
							});
					},
		...
		if (
			frm.doc.approved_count &&
			(!frm.doc.inventory_delta || frm.doc.inventory_delta === 0)
		) {
...
function fetch_meta(frm) {
	...
	d.$body.html(
		`<div style="height:400px; overflow:auto;"><pre style="white-space:pre-wrap;word-break:break-word;">Loading...</pre></div>`,
	);
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			if (mode === "download") {
				...
				if (frm.doc.__islocal) {
					frappe.msgprint(
						"Please save the document first so the PDF and instruction image can be attached here.",
					);
					return;
				}
...
function download_shooting_kit(frm, marker_mm, aruco_dict, pad_mm) {
	const base =
		"/api/method/repair_portal.inventory.doctype.pad_count_intake.pad_count_intake_api.generate_shooting_kit_preview";
